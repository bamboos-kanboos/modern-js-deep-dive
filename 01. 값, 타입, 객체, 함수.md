...
> js 엔진이 다르게 취급하는 데이터의 종류
> 
> - 다르게 취급 = 메모리 확보, 2진수 변환 결과, 읽어들여 해석하는 방식
> 
> js 는 변수 선언 시 타입을 정하지 않기 때문에(var, let 등 사용) 값을 할당하는 시점에 값에 해당하는 타입을 부여하는 동적 타입 언어
> 
1. 원시형(Primitive)
...
3. 함수 (Function)
    ...
    - 함수 생성 방식
        - 등등
        - 생성자: new Function → 비추천.
            - 런타임에 함수를 생성해서 비쌈
            - 클로저 생성 X
            - 문자열을 받아서 함수를 생성하므로 인젝션 등 보안 취약 (eval 보다는 좀 낫다…주변
    - 헷갈릴 만한 것들 실험
    ...
    - 호이스팅
      - 변수와 함수 선언이 스코프의 최상단으로 끌어올려지는 것처럼 동작하는 현상
      - `var`는 `undefined`로 초기화되어 선언 전에 접근 가능
      - `let`, `const`도 호이스팅되지만 **TDZ(Temporal Dead Zone)** 때문에 초기화 전에는 접근할 수 없음
        ...
        - 원인
            - 아래에서 `const sourceTextureManager`를 선언하는 순간 함수 전체에서 지역 변수로 인식됨(Shadowing)
            - 하지만 아직 초기화되지 않은 상태(TDZ)이므로 위에서 접근하면 에러 발생
